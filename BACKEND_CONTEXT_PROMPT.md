...
**Current Status:** Backend API is functional with Auth, Coach, LessonPackage, and Reservation CRUD operations. The database includes a new `Sports` catalog table. Running via Docker Compose. No frontend yet (CORS configured for React on port 3000).
...
**Sports** (extends BaseEntity):
- `Name` (string, required, max length 100, unique index)
- Nav: `Coaches` (1:N)

**Coach** (extends BaseEntity):
- `UserId` (Guid, FK to User — 1:1)
- `SportId` (Guid, FK to Sports — 1:N)
- `Bio?`, `HourlyRate` (decimal), `Experience` (int, years), `IsApproved` (default false), `AverageRating` (decimal), `Location?`, `Iban?`
- Nav: `User`, `Sports`, `Packages` (1:N), `Reservations` (1:N)
...
**Auth:**
- `RegisterRequestDto`: Name, Email, Password, RoleId (int), SportId? (Guid?)
- `AuthResponseDto`: Id, Name, Email, Message, RoleId, Sport (string)
- `LoginRequestDto`: Email, Password
- `LoginResponseDto`: Token, UserId, Name, Role

**Coach:**
- `CoachResponseDto`: Id, FullName (mapped from User.Name), Sport (mapped from Sports.Name), Bio?, HourlyRate, Experience, AverageRating, Location?
- `UpdateCoachProfileDto`: SportId (Guid), Bio?, HourlyRate, Experience, Location?, Iban?
...
- `ICoachRepository`: + GetAllCoachesWithDetailsAsync(), GetCoachesBySportAsync(string sport), GetCoachWithPackagesAsync(Guid), GetTopRatedCoachesAsync(int), GetCoachByUserIdAsync(Guid)
...
- `ICoachService`: GetAllCoachesAsync(), GetCoachByIdAsync(Guid), GetCoachesBySportAsync(string sport), UpdateProfileAsync(Guid userId, UpdateCoachProfileDto)
...
**AuthService:**
- Registration: Checks email uniqueness, hashes password with BCrypt, creates User. If RoleId==2 (Coach), also creates a Coach entity with `SportId`. Saves to DB.
...
**CoachService:** Uses AutoMapper. GetAll includes User and Sports details. UpdateProfile finds coach by UserId, updates `SportId` and other fields, saves.
...
- `CoachProfile`: Coach → CoachResponseDto, maps `FullName` from `src.User.Name` and `Sport` from `src.Sports.Name`.
...
- DbSets: Users, Roles, Coaches, Sports, LessonPackages, Reservations, Payments, AuditLogs
- `OnModelCreating`: Applies all IEntityTypeConfiguration from assembly. Adds **global query filters** for soft delete on User, Coach, Sports, LessonPackage, Reservation, Payment.
...
**SportConfiguration:** PK=Id, Name required/max100, unique index on Name.

**CoachConfiguration:** Location max255, Iban max34, Bio max1000, HourlyRate precision(18,2), AverageRating precision(3,2). Coach→Packages: 1:N via CoachId, Cascade delete. Coach→Sports: N:1 via SportId, Restrict delete.
...
**CoachRepository:** GetCoachesBySportAsync uses `.Include(c => c.Sports)` and filters by `c.Sports.Name` (case-insensitive) AND IsApproved. GetCoachWithPackagesAsync includes Packages+User. GetTopRatedCoachesAsync filters IsApproved, orders by AverageRating desc. GetAllCoachesWithDetailsAsync includes User. GetCoachByUserIdAsync filters by UserId.
...
3. `20260504084159_AddSportsTable` — Dropped the old `Sport` string column on Coaches, created `Sports` table, added `SportId` column to Coaches, and created the foreign key constraint. Manually added an 'Uncategorized' default sport insert query to avoid foreign key errors on existing coaches.
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Port=5433;Database=meetinsportDb;Username=admin;Password=admin123;"
  },
  "JwtSettings": {
...
```
**Note:** The appsettings connection string matches the .env settings. It is injected into Docker Compose.
...
- Sports → Coach: Restrict
- AuditLog → User: SetNull

**Soft delete:** All BaseEntity entities (User, Coach, Sports, LessonPackage, Reservation, Payment) use soft delete via IsDeleted flag + global query filters.
...
2. If RoleId=2 (Coach), a Coach entity is auto-created with default values (including the provided `SportId`)
...
└── src/
    ...
    │   ├── Entities/
    │   │   ├── User.cs, Coach.cs, Role.cs, Sports.cs
    ...
    │   ├── DependencyInjection.cs
    │   ├── Configurations/ (User, Role, Coach, Sport, LessonPackage, Reservation, Payment, AuditLog)
    ...
    │   └── Migrations/ (InitialCreate, MakeCancelledAtNullable, AddSportsTable)
    │
    └── MeetInSport.WebApi/
        ├── Program.cs
        ├── appsettings.json
...
| GET | `/api/v1/coaches/sport/{sport}` | Anonymous | Search coaches by sport name |
...
2. **Coach auto-creation on registration:** When a user registers with RoleId=2, a Coach entity is automatically created with default placeholder values (SportId from request if any, other fields set to defaults).
